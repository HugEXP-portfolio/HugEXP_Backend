...
public class QuestController {
    ...
    public ResponseEntity<ApiResponse<?>> getAllQuests(
        ...
        return ResponseEntity.ok().body(ApiResponse.success("All Quests successfully retrieved.", response));
    }

    @PutMapping("/complete/{userQuestId}")
    @Operation(summary = "퀘스트 완료 처리", description = "일일 퀘스트를 완료한다.")
    public ResponseEntity<ApiResponse<?>> completeQuest(
            @AuthenticationPrincipal CustomUserDetails userDetails,
            @PathVariable Long userQuestId) {
        User user = userDetails.getUser();
        UserQuestResponse response = questService.completeQuest(user, userQuestId);
        return ResponseEntity.ok().body(ApiResponse.success("Quest successfully completed.", response));
    }
}
